...
`tree()` likes to work factor or numerical variables (or both), and it complains when there is missing data.  If you've got `NA` values in your data, consider removing them all:

```{r}
s2 <- subset(m111survey, complete.cases(m111survey) == T)
nrow(s2)  # the previous operation removed three rows
```
...
```{r}
trMod <- tree(sex ~ seat + fastest + GPA, data = s2)
```
...
```{r}
trFast <- tree(fastest ~ sex + seat + height + GPA, data = s2)
```
...
The default values of `minuct`, minsize` and `mindev` are 5, 10  and 0.01 respectively.  Thus the tree obtained without recourse to `tree.control()`:

```{r eval = F}
trSex <- tree(sex ~ ., data = s2)
```
